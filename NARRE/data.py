...
def prepare_data(
    ...
    user_review_counts = [len(user_reviews_dict[u]) for u in range(len(user_values))]
    item_review_counts = [len(item_reviews_dict[i]) for i in range(len(item_values))]
    sent_lens = [max(1, len(r.split())) for r in clean_train_reviews]
    ...
    r_max_len = _compute_percentile_length(sent_lens, p_review, minimum=1)

    user_reviews_arr = np.zeros((len(user_values), u_max_r, r_max_len), dtype=np.int64)
    item_reviews_arr = np.zeros((len(item_values), i_max_r, r_max_len), dtype=np.int64)
    user_item2id = np.full((len(user_values), u_max_r), fill_value=len(item_values), dtype=np.int64)
    item_user2id = np.full((len(item_values), i_max_r), fill_value=len(user_values), dtype=np.int64)

    for u in range(len(user_values)):
        reviews = user_reviews_dict[u] if user_reviews_dict[u] else ["<unk>"]
        for ridx, txt in enumerate(reviews[:u_max_r]):
            user_reviews_arr[u, ridx] = np.array(_encode_tokens(txt.split(), vocab, r_max_len), dtype=np.int64)
    ...
    for i in range(len(item_values)):
        ...
        for ridx, txt in enumerate(reviews[:i_max_r]):
            item_reviews_arr[i, ridx] = np.array(_encode_tokens(txt.split(), vocab, r_max_len), dtype=np.int64)
    ...
    if pretrained_emb_path:
        ...
        if emb_format == "npy":
            ...
            if w2v.shape[0] < vocab_size:
                pad = np.random.uniform(-1.0, 1.0, size=(vocab_size - w2v.shape[0], word_dim)).astype(np.float32)
