...
@torch.no_grad()
def evaluate(model: DeepCoNN, loader: DataLoader, pdata, device: torch.device):
    model.eval()
    total_mse = 0.0
    total_mae = 0.0
    total_n = 0
...
def main(args):
    ...
    model = DeepCoNN(args).to(args.device)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="DeepCoNN training (paper-aligned config)")
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Path to pretrained embeddings: .bin/.bin.gz (word2vec), .txt/.vec (GloVe), or .npy matrix.",
    )
    parser.add_argument(
    ...
    parser.add_argument("--model_name", type=str, default="DeepCoNN")
    ...
    args.model_save_path = os.path.join(
        args.output_dir,
        f"{args.dataset_name}_{args.model_name}_{args.seed}.pth",
    )
