...
def build_batch(pdata, uids: torch.Tensor, iids: torch.Tensor, device: torch.device):
    user_docs = pdata.user_docs[uids].to(device)
    item_docs = pdata.item_docs[iids].to(device)

    user_neighbors = pdata.user_neighbors[uids].to(device)
    item_neighbors = pdata.item_neighbors[iids].to(device)
    user_item_user = pdata.user_item_user[uids].to(device)
    item_user_item = pdata.item_user_item[iids].to(device)
...
@torch.no_grad()
def evaluate(model: RMG, loader: DataLoader, pdata, device: torch.device):
    model.eval()
    total_mse = 0.0
    total_mae = 0.0
    total_n = 0
...
def main(args):
    ...
    pdata = prepare_data(
        ...
        max_review_user=args.max_review_user,
        max_review_item=args.max_review_item,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="RMG training (paper-aligned setting)")
    ...
    # Paper experimental setting from your excerpt.
    parser.add_argument("--word_dim", type=int, default=300)
    parser.add_argument("--cnn_filters", type=int, default=150)
    parser.add_argument("--cnn_window", type=int, default=3)
    parser.add_argument("--id_emb_dim", type=int, default=100)
    parser.add_argument("--dropout", type=float, default=0.25)
    parser.add_argument("--batch_size", type=int, default=20)
    parser.add_argument("--lr", type=float, default=1e-3)

    # Sequence construction used by the released baseline notebook.
    parser.add_argument("--max_sent_len", type=int, default=40)
    parser.add_argument("--max_sents", type=int, default=15)
    parser.add_argument("--max_review_user", type=int, default=40)
    parser.add_argument("--max_review_item", type=int, default=50)
    parser.add_argument("--max_neighbor", type=int, default=75)
    parser.add_argument("--max_vocab", type=int, default=50000)

    parser.add_argument("--attn_dim", type=int, default=100)
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Path to pretrained embeddings: GoogleNews .bin/.bin.gz, GloVe .txt/.vec, or .npy matrix.",
    )
    parser.add_argument(
    ...
    parser.add_argument("--model_name", type=str, default="RMG")
