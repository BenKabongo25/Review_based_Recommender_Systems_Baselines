...
def build_batch(pdata, uids: torch.Tensor, iids: torch.Tensor, device: torch.device):
    ...
    return user_reviews, item_reviews, uids.to(device), iids.to(device)


@torch.no_grad()
def evaluate(model: MPCN, loader: DataLoader, pdata, device: torch.device):
    model.eval()
    total_mse = 0.0
    total_mae = 0.0
    total_n = 0
    ...
    for uids, iids, scores in loader:
        ...
        user_reviews, item_reviews, _, _ = build_batch(pdata, uids, iids, device)
        preds = model(user_reviews, item_reviews)
...
def main(args):
    ...
    model = MPCN(args).to(args.device)
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        for uids, iids, scores in train_loader:
            ...
            user_reviews, item_reviews, _, _ = build_batch(pdata, uids, iids, args.device)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MPCN training")
    ...
    parser.add_argument("--review_num", type=int, default=20)
    parser.add_argument("--review_len", type=int, default=30)

    parser.add_argument("--num_pointers", type=int, default=3)
    parser.add_argument("--pointer_agg", type=str, default="neural", choices=["add", "concat", "neural"])

    parser.add_argument("--model_name", type=str, default="MPCN")
