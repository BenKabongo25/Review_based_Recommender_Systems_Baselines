...
class SGDNDataset:
    def __init__(
        ...
        self.possible_rating_values = np.unique(np.array(self.train_datas[2], dtype=np.float32))
        self.num_rating = len(self.train_datas[2])
        ...
        def _make_labels(ratings):
            labels = torch.LongTensor(np.searchsorted(self.possible_rating_values, ratings)).to(device)
            return labels

        self.train_enc_graphs = self._generate_enc_graph(
            train_rating_pairs,
            train_rating_values,
            train_ui_raw,
            add_support=True,
        ...
        rating_counter = train_df["rating"].astype(np.float32).value_counts().to_dict()
        self.rating_split = [int(rating_counter.get(float(r), 0)) for r in sorted(self.possible_rating_values, reverse=True)]

        self.train_labels = _make_labels(train_rating_values)
        self.train_truths = torch.FloatTensor(train_rating_values).to(device)

        self.degree_u, self.degree_i = self._cal_degree(self.train_enc_graphs[0])
        ...
        self.test_dec_subgraphs, self.test_ratings = self._generate_dec_subgraphs(
            test_rating_pairs,
            self.test_truths,
            review_feat=None,
            test_ui_raw=test_ui_raw,
        )

        def _npairs(graph):
            rst = 0
            for r in self.possible_rating_values:
                rname = to_etype_name(r)
                rst += graph.number_of_edges(str(rname))
            return rst

        print(
            "Train enc graph: \t#user:{}\t#item:{}\t#pairs:{}".format(
                self.train_enc_graphs[0].number_of_nodes("user"),
                self.train_enc_graphs[0].number_of_nodes("item"),
        ...
        print(
            "Valid enc graph: \t#user:{}\t#item:{}\t#pairs:{}".format(
                ...
                self.valid_enc_graphs[0].number_of_nodes("item"),
        ...
        print(
            "Test enc graph: \t#user:{}\t#item:{}\t#pairs:{}".format(
                ...
                self.test_enc_graphs[0].number_of_nodes("item"),
    ...
    def _cal_degree(self, graph):
        degree_u = torch.zeros(self.user_feature_shape[0])
        degree_i = torch.zeros(self.item_feature_shape[0])
    ...
    def _load_review_feat(self, review_feat_path):
        try:
            ...
            raise FileNotFoundError(f"Review feature file not found: {review_feat_path}") from exc
    ...
    def _process_split_df(self, split_df: pd.DataFrame):
        ...
        review_text = split_df.get("review", pd.Series([""] * len(split_df))).astype(str).tolist()

        user_nid = [self.user2nid[u] for u in user_raw]
        item_nid = [self.item2nid[i] for i in item_raw]

        return user_nid, item_nid, rating, review_text, user_raw, item_raw
    ...
    def _generate_pair_value(self, sub_dataset):
        if sub_dataset == "all_train":
            ...
            user_raw = self.train_datas[4] + self.valid_datas[4]
            item_raw = self.train_datas[5] + self.valid_datas[5]
        elif sub_dataset == "train":
            user_id, item_id, rating, _, user_raw, item_raw = self.train_datas
        elif sub_dataset == "valid":
            user_id, item_id, rating, _, user_raw, item_raw = self.valid_datas
        else:
            user_id, item_id, rating, _, user_raw, item_raw = self.test_datas

        rating_pairs = (np.array(user_id, dtype=np.int64), np.array(item_id, dtype=np.int64))
    ...
    def _generate_enc_graph(self, rating_pairs, rating_values, ui_raw, add_support=False):
        ...
        if record_size == 0:
            raise ValueError("Empty split when generating encoder graph.")
        review_feat_list = self._lookup_review_feat(ui_raw)
        ...
        graphs = []
        review_data_dict = {}
        for _ in range(self.num_factor):
            data_dict = {}
            for rating in self.possible_rating_values:
                ...
                data_dict.update(
                    {
                        ("user", str(rating_name), "item"): (rrow, rcol),
                        ("item", f"rev-{rating_name}", "user"): (rcol, rrow),
                    }
                )
                review_data_dict[str(rating_name)] = review_feat_list[ridx]
            graph = dgl.heterograph(data_dict, num_nodes_dict=num_nodes_dict)
            graphs.append(graph)

        self.review_data_dict = review_data_dict

        assert len(rating_pairs[0]) == sum([graphs[0].number_of_edges(et) for et in graphs[0].etypes]) // 2
    ...
    def _generate_dec_graph(self, rating_pairs, ui_raw, review_feat=None):
        ...
        user_item_ratings_coo = sp.coo_matrix(
            (ones, rating_pairs), shape=(self.num_user, self.num_item), dtype=np.float32
        )
    ...
    def _generate_dec_subgraphs(self, test_rating_pairs, test_rating_values, review_feat=None, test_ui_raw=None):
        gs, test_ratings = [], []
        ...
        for idx in idxs:
            ...
            if review_feat is not None and test_ui_raw is not None:
                ui = [test_ui_raw[i] for i, flag in enumerate(idx.tolist()) if flag]
                feat = self._lookup_review_feat(ui)
                g.edata["review_feat"] = feat
