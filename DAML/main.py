...
@torch.no_grad()
def evaluate(model: DAML, loader: DataLoader, pdata, device: torch.device):
    model.eval()
    total_mse = 0.0
    total_mae = 0.0
    total_n = 0
...
def main(args):
    ...
    model = DAML(args).to(args.device)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="DAML training (paper-style simplified runner)")
    ...
    parser.add_argument("--model_name", type=str, default="DAML")
